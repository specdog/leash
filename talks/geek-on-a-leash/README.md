...
An editable 49-slide, one-hour Rust Tuesdays talk about Leash: a Rust boundary between agent intent and physical motion.

The deck spends approximately 5 minutes on the DimOS inspiration and the case for a conceptual Rust port, 47 minutes in pinned Leash source and CUDA kernels, 5 minutes on a bounded live UGV demonstration, and 3 minutes on questions. Thirty-eight slides show source directly: the DimOS module/blueprint surface, Leash's Rust authority boundary, newtypes, checked domain values, typestate, `PhantomData`, compile-fail doctests, traits and associated types, static and dynamic dispatch, ownership across threads, `Drop`, bounded generics, atomics and memory ordering, `Option` ownership operations, Serde wire contracts, error conversion, ROS2 frame types, checked CUDA artifacts, kernel indexing and masking, device-buffer ownership, the isolated CUDA `unsafe` launch, reduction atomics, predictive updates, end-to-end break-even measurements, shadow parity, and CPU fallback. Leash is presented as an independent implementation inspired by DimOS's agent-native direction—not a DimOS fork or compatibility claim. Qualia appears once as the slower mission/ontology layer; it is not presented as part of Leash.

## Artifacts

- `output/geek-on-a-leash.pptx` — editable PowerPoint with 49 speaker-note sections
