...
fn compact_telemetry_value(harness: &Harness) -> Value {
    ...
    json!({
        ...
        "odometry_pose": telemetry.get("odometry_pose").cloned().unwrap_or(Value::Null),
...
async fn agent_console_capability(
    ...
    let (capability, args) = validate_agent_console_capability(&harness, req)?;
    let runtime = AgentRuntime::from_env(harness, CapabilityPermissions::from_env()?)?;
...
fn validate_agent_console_capability(
    harness: &Harness,
    req: AgentCapabilityReq,
...
mod tests {
    ...
    async fn compact_telemetry_keeps_qualia_inputs_and_drops_dense_surfaces() {
        ...
        assert!(value.get("odometry_pose").is_some());
    ...
    async fn qualia_inputs_omit_voxels_and_keep_relative_odometry_contract() {
        ...
        assert!(value.get("sensors").is_some());
        assert!(value.get("localization").is_some());
        assert!(value.get("localization_provider").is_some());
        assert!(value.get("odometry_pose").is_some());
        assert!(value.get("voxel_grid").is_none());
        assert!(value.get("occupancy_grid").is_none());
    }
    ...
    async fn agent_console_runs_persisted_turns_and_only_probes_observe_capabilities() {
        ...
        let runtime = AgentRuntime::new(
            harness.clone(),
            AgentSessionStore::new(state_root.join("agent")),
            CapabilityPermissions::default(),
        );
        let run = runtime
            .run_prompt("show current state", Some("headful-test"), false)
            .unwrap();
        assert_eq!(run.session.id, "headful-test");

        let snapshot = runtime.snapshot(Some("headful-test"), 6).unwrap();
        assert_eq!(snapshot.selected_session.unwrap().turns.len(), 1);

        let (capability, args) = validate_agent_console_capability(
            &harness,
            AgentCapabilityReq {
        ...
        .unwrap();
        assert_eq!(
            runtime.invoke_capability(&capability, args).unwrap()["ok"],
            true
        );

        let denied = validate_agent_console_capability(
            &harness,
            AgentCapabilityReq {
