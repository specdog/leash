...
pub enum PawprintTransport {
    Http,
    Mcp,
}

impl PawprintTransport {
...
pub struct TransportBinding {
    pub kind: PawprintTransport,
    pub listen: Option<SocketAddr>,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[cfg_attr(feature = "mcp", derive(schemars::JsonSchema))]
pub struct PawprintModule {
    pub name: String,
...
pub struct Pawprint {
    ...
    pub modules: Vec<PawprintModule>,
    pub command: String,
}
...
impl Pawprint {
    pub fn validate(&self) -> Result<()> {
        if self.hardware_required != self.profile.is_physical() {
            bail!(
                "pawprint '{}' hardware metadata does not match profile '{}'",
                self.name,
                self.profile.as_str()
            );
        }
        ...
        for module in &self.modules {
            if !graph_modules.contains(module.name.as_str()) {
                bail!(
                    "pawprint '{}' references unknown module '{}'",
                    self.name,
                    module.name
                );
            }
...
pub fn built_in_pawprints() -> Vec<Pawprint> {
    vec![
        sim_http_pawprint(),
        sim_mcp_pawprint(),
        bridge_compat_http_pawprint(),
        waveshare_ugv_http_pawprint(),
    ]
}

pub fn find_pawprint(name: &str) -> Option<Pawprint> {
    built_in_pawprints()
        .into_iter()
        .find(|pawprint| pawprint.name == name)
}

pub fn resolve_pawprint(name: &str) -> Result<Pawprint> {
    let Some(pawprint) = find_pawprint(name) else {
        let names = built_in_pawprints()
            .into_iter()
            .map(|pawprint| pawprint.name)
            .collect::<Vec<_>>()
            .join(", ");
        bail!("unknown pawprint '{name}'; expected one of: {names}");
    };
    pawprint.validate()?;
    Ok(pawprint)
}

fn sim_http_pawprint() -> Pawprint {
    Pawprint {
        name: "sim-http".to_string(),
        description: "Simulation HTTP runtime with WebSocket telemetry".to_string(),
        profile: Profile::Sim,
        transport: TransportBinding {
            kind: PawprintTransport::Http,
...
fn bridge_compat_http_pawprint() -> Pawprint {
    Pawprint {
...
fn waveshare_ugv_http_pawprint() -> Pawprint {
    Pawprint {
...
fn module_refs(profile: Profile) -> Vec<PawprintModule> {
    let driver = match profile {
        Profile::Sim => ("sim-driver", false),
        Profile::WaveshareUgv => ("waveshare-ugv-driver", true),
    };
    vec![
        pawprint_module("harness-runtime", "runtime", true, false),
        pawprint_module(driver.0, "driver", true, driver.1),
        pawprint_module("telemetry", "telemetry", true, false),
    ]
}
...
fn socket(value: &str) -> SocketAddr {
    SocketAddr::from_str(value).expect("built-in pawprint socket is valid")
}
...
mod tests {
    ...
    #[test]
    fn built_in_pawprints_validate() {
        let pawprints = built_in_pawprints();
        assert!(pawprints.iter().any(|pawprint| pawprint.name == "sim-http"));
        assert!(pawprints.iter().any(|pawprint| pawprint.name == "sim-mcp"));
        assert!(pawprints
            .iter()
            .any(|pawprint| pawprint.name == "bridge-compat-http"));
        assert!(pawprints
            .iter()
            .any(|pawprint| pawprint.name == "waveshare-ugv-http"));

        for pawprint in pawprints {
            pawprint.validate().unwrap();
        }
    }

    #[test]
    fn unknown_pawprint_reports_known_names() {
        let err = resolve_pawprint("missing").unwrap_err().to_string();
        assert!(err.contains("unknown pawprint 'missing'"));
        assert!(err.contains("sim-http"));
    }

    #[test]
    fn physical_pawprint_resolves_before_refusing_without_gate() {
        let pawprint = resolve_pawprint("waveshare-ugv-http").unwrap();
        let config = resolve_config(ConfigRequest {
            config_path: None,
            pawprint: Some(pawprint.profile),
            pawprint_defaults: pawprint.config_overrides,
            env: BTreeMap::new(),
