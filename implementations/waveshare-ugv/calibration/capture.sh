...
usage() {
  cat <<'EOF'
Usage: capture.sh --phase PHASE --profile FILE --env-file FILE --output FILE --replay-output FILE [options]
...
This recorder never issues drive, motor, or velocity commands. It sends stop
before and after capture. For a motion phase, a present operator must drive only
through an already-authorized Leash control surface.
EOF
}
...
if [[ "$phase" != "stationary" && "$operator_confirmed" != true ]]; then
  echo "motion capture requires --operator-confirmed after an on-site safety check" >&2
  exit 1
fi
