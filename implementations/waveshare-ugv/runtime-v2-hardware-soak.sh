...
max_gpu_temp_c="$(awk '{for (i=1;i<=NF;i++) if (tolower($i) ~ /^gpu@/) {value=tolower($i); sub(/^gpu@/,"",value); sub(/c.*/,"",value); if (value>max) max=value}} END {printf "%.3f", max+0}' "$tegrastats_file")"
(( tegrastats_samples > 0 && max_ram_mb > 0 )) || die "tegrastats RAM samples were unavailable"
awk -v value="$max_gpu_temp_c" 'BEGIN {exit !(value > 0)}' || die "tegrastats GPU temperature samples were unavailable"
